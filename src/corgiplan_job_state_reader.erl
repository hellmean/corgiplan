...
%% API
-export([stop/1, start_link/0, get_job_state/1, get_execution_results/1]).
...
-spec get_execution_results(JobServerName::term()) -> {ok, [#corgiplan_execution_result{}]}.
get_execution_results(JobServerName) ->
    gen_server:call({global, corgiplan_job_state_reader}, {get_execution_results, JobServerName}).

stop(Name) ->
    gen_server:call(Name, stop).

start_link() ->
    gen_server:start_link({global, corgiplan_job_state_reader},
